    ...
    $('#dg').datagrid({
        ...
        columns: [[
            ...
            {field: 'contact', title: '联系电话', width: 100, align: 'left'},
            {
                ...
                formatter: function (value, row, index) {
                    return "<a href='javascript:houYesPass(" + row.id + ")'>待审核</a>";
                }
            }
        ]]
    });
...
function houYesPass(id) {
    ...
    $.post("isPassHouse?ispass=0", {"id": id}, function (data) {
        if (data.result > 0) {
            $('#dg').datagrid('reload');
        } else {
            $.messager.alert('提示框', '修改状态失败')
        }
    }, "json")
...
function allispassHouse() {
    ...
    $.post("upAllisPassHouse?ispass=0",{"id":id},function (da) {
        if(da.result>0){
            $('#dg').datagrid('reload');
        }else {
